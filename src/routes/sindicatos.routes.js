...
router.post('/', requireRole('rrhh', 'admin'), async (req, res, next) => {
  try {
    ...
    const ins = await query(
      `INSERT INTO sindicatos (codigo, nombre, pct_empleado, pct_patronal, pct_antig_por_anio, nota, tiene_adicional_titulo, pres_base, titulo_secundario, titulo_universitario, updated_by)
       ...
       ON CONFLICT (codigo) DO UPDATE SET nombre=EXCLUDED.nombre, pct_empleado=EXCLUDED.pct_empleado, pct_patronal=EXCLUDED.pct_patronal,
         pct_antig_por_anio=EXCLUDED.pct_antig_por_anio, nota=EXCLUDED.nota, tiene_adicional_titulo=EXCLUDED.tiene_adicional_titulo, pres_base=EXCLUDED.pres_base, titulo_secundario=EXCLUDED.titulo_secundario, titulo_universitario=EXCLUDED.titulo_universitario, updated_by=EXCLUDED.updated_by, updated_at=now()
       RETURNING *`,
      [String(b.codigo).toUpperCase(), b.nombre, b.pctEmpleado || 0, b.pctPatronal || 0, b.pctAntigPorAnio || 1, b.nota || null, (tSec > 0 || tUni > 0), b.presBase || 'basico', tSec, tUni, req.user.dni]
    );
...
router.put('/:id', requireRole('rrhh', 'admin'), async (req, res, next) => {
  try {
    ...
    const tSec = Number(b.tituloSecundario) || 0, tUni = Number(b.tituloUniversitario) || 0;
    const r = await query(
      `UPDATE sindicatos SET nombre=$1, pct_empleado=$2, pct_patronal=$3, pct_antig_por_anio=$4, nota=$5, tiene_adicional_titulo=$6, pres_base=$7, titulo_secundario=$8, titulo_universitario=$9, updated_by=$10, updated_at=now() WHERE id=$11 RETURNING *`,
      [b.nombre, b.pctEmpleado || 0, b.pctPatronal || 0, b.pctAntigPorAnio || 1, b.nota || null, (tSec > 0 || tUni > 0), b.presBase || 'basico', tSec, tUni, req.user.dni, req.params.id]
    );

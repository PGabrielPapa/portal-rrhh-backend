...
// Para managers: ids de su equipo (con delegaciones). Para RR.HH./admin: null = sin restricción.
async function scopeIds(req) {
  if (esRH(req.user.role)) return null;
  return [...await equipoEfectivo(req.user, 'desempeno')];
}
...
// GET /api/desempeno?anio= — fichas del año (RR.HH.: todas; manager: solo su equipo).
router.get('/', async (req, res, next) => {
  try {
    ...
    if (ids) { params.push(ids); filtro = ` AND d.empleado_id = ANY($${params.length})`; }
    const { rows } = await query(
      `SELECT d.*, e.nom, e.leg_num, em.nombre AS empresa
        ...
        WHERE d.anio=$1${filtro} ORDER BY em.nombre, e.nom`, params);
    res.json(rows.map(map));
  } catch (e) { next(e); }
});

// GET /api/desempeno/9box?anio= — grilla 9-box (RR.HH.: todos; manager: su equipo).
router.get('/9box', async (req, res, next) => {
  try {
    const anio = Number(req.query.anio) || new Date().getFullYear();
    const ids = await scopeIds(req);
    if (ids && !ids.length) return res.json({ anio, celdas: {} });
    const params = [anio]; let filtro = '';
    if (ids) { params.push(ids); filtro = ` AND d.empleado_id = ANY($${params.length})`; }
    const { rows } = await query(
      `SELECT d.empleado_id, d.desempeno, d.potencial, e.nom, e.leg_num
         FROM desempeno d JOIN empleados e ON e.id=d.empleado_id
        WHERE d.anio=$1 AND d.desempeno IS NOT NULL AND d.potencial IS NOT NULL${filtro}`, params);
    const celdas = {};
...
// Valida que el manager solo acceda a su equipo; RR.HH./admin sin restricción.
async function puedeVer(req, empleadoId) {
  const ids = await scopeIds(req);
  return !ids || ids.includes(Number(empleadoId));
}

// GET /api/desempeno/:empleadoId?anio=
router.get('/:empleadoId', async (req, res, next) => {
  try {
    if (!await puedeVer(req, req.params.empleadoId)) return res.status(403).json({ error: 'Sin permisos sobre ese empleado.' });
    const anio = Number(req.query.anio) || new Date().getFullYear();
